...
BRANCH="playstatus-$VERSION-build$BUILD_NUMBER"

git clone "https://x-access-token:$TAP_TOKEN@github.com/$TAP_REPOSITORY.git" "$WORK_DIR/tap"
mkdir -p "$WORK_DIR/tap/Casks"
scripts/render-cask.sh "$VERSION" "$BUILD_NUMBER" "$SHA256" > "$WORK_DIR/tap/Casks/playstatus.rb"
...
if git -C "$WORK_DIR/tap" diff --cached --quiet; then
  echo "Cask already matches $VERSION build $BUILD_NUMBER; no pull request needed."
  exit 0
fi
git -C "$WORK_DIR/tap" -c user.name="playstatus-release[bot]" -c user.email="playstatus-release[bot]@users.noreply.github.com" \
  commit -m "playstatus $VERSION build $BUILD_NUMBER"
...
GH_TOKEN="$TAP_TOKEN" gh pr create \
  ...
  --base main \
  --head "$BRANCH" \
  --title "playstatus $VERSION build $BUILD_NUMBER" \
  --body "Automated cask update for PlayStatus $VERSION build $BUILD_NUMBER.\n\nSHA-256: \`$SHA256\`"
